...
int sum(vector<vector<int>> v, int sr, int sc, int er, int ec) {
    ...
    // Calculate the sum for the submatrix from (sr, sc) to (er, ec)
    int submatrixSum = prefix[er][ec];
    
    if (sr > 0) submatrixSum -= prefix[sr-1][ec];
    if (sc > 0) submatrixSum -= prefix[er][sc-1];
    if (sr > 0 && sc > 0) submatrixSum += prefix[sr-1][sc-1];
...
int main() {
    ...
    cout << sum(v, 0, 0, 1, 1) << endl;  // Output: 11
    cout << sum(v, 2, 2, 3, 4) << endl;  // Output: 38
    cout << sum(v, 1, 2, 3, 3) << endl;  // Output: 38
